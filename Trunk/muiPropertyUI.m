...
%%
        function obj = editProperties(obj,nrec)
            %for class obj get the open properties and provide input dlg to
            %allow user to edit property values (assumes all numerical)
            if nargin<2
...
%%
		function displayProperties(obj,src)
			%display properties as a table on PropertyTab
...
%%
        function obj = setTabProps(obj,mobj,classname)
