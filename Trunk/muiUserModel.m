...
%   ctWaveModel.m
% PURPOSE
%    Class for data that is derived using hte muiManipUI interface included
...
%--------------------------------------------------------------------------
    methods
        function createVar(obj,gobj,mobj)
            ...
            isvalid = isValidEqnSelection(obj,gobj,inp);
            if ~isvalid, return; end
            
            waitbar(0.2)
            [XYZT,props,ok] = getData(obj,gobj,mobj.Cases);
            if ok<1, return; end 
            waitbar(0.8)
            ...
            close(hw)
            setEqnData(obj,mobj,gobj,varout);            
        end  
%%
...
%%
        function isvalid = isValidEqnSelection(~,gobj,inp)
            ...
            for i=1:3  
                ...
                if inp.isXYZT(i) && gobj.UIselection(i).caserec<1
...
%%
        function [XYZT,props,ok] = getData(obj,gobj,muicat)
            %get the data to be used in the equation or function
            ok = 1;
            uisel = gobj.UIselection;
            nvar = length(uisel);
            ...
            for i=1:nvar                
                ...
                if uisel(i).caserec>0  %case assigned in selection
                    props(i) = getProperty(muicat,uisel(i),'dstable');
...
%%
        function setEqnData(obj,mobj,gobj,var)
            ...
            elseif isa(var,'dstable')                  %need to test this****        
                    inputxt = setMetaData(gobj,eqn);
                    save2obj(obj,mobj,var,inputxt);
            else
                inputxt = setMetaData(gobj,eqn);
                setdsp2save(obj,mobj,var,inputxt);
            end
            %--------------------------------------------------------------
            function inputxt = setMetaData(gobj,eqn)
                %set metadata for output
                uisel=gobj.UIselection;
                idx = find(~[uisel.caserec])==0;  %index to used XYZ variables
                inputxt = sprintf('Used: %s with inputs:',eqn);
                for j=1:length(idx)
                    inputxt = sprintf('%s\n%s',inputxt,(uisel(idx(j)).desc));
                end
            end
        end
%%                
        function setdsp2save(obj,mobj,var,inputxt)
            ...
            sz = size(results{1})>1;   %dimensions with more than single value
