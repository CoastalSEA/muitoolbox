...
%-----------------------------------------------------------------------------------------
% modified to handle NaN values Aug 2022, IHT
...
for Tmax=5:maxT
    ...
    for tt = 1:Tmax
        ...
        if ~isinf(delta)
            w  = exp(-aph*([N-2:-1:0]))*(1-exp(-aph))/(1-exp(-aph*(N-1)));
            w1 = exp(-aph*([N-1:-1:0]))*(1-exp(-aph))/(1-exp(-aph*N));
            for qq=1:lq
                mcord(tt,qq)=sum(w.*(abs(ddVd').^q(qq)),'omitnan')/sum(w1.*(abs(VVVd).^q(qq)),'omitnan');
            end
        else
            for qq=1:lq
                mcord(tt,qq)=mean(abs(ddVd).^q(qq),'omitnan')/mean(abs(VVVd).^q(qq),'omitnan');
            end
        end
    end
    mx = mean(log10(x));
...
% figure('Name','Hurst Plot','Tag','StatFig') %verify scaling
% loglog(x,mcord,'x-')
mH = mean(H)'./q(:);
if nargout == 2
    sH = std(H)'./q(:);
