...
%   
    ...
    %select tile spacing and padding to use in composite tiled plot
    spacetxt = {"loose", "compact","tight","none"};
    varargin =  {'FigureTitle','Select variables',...
                 'PromptText','Select Tile spacing and padding',...
                 'InputFields',{'Tile spacing','Padding'},...
                 'InputOrder',{'',''},...
                 'Style',repmat({'popupmenu'},1,2),...
                 'ControlButtons',{},...                            
                 'DefaultInputs',{spacetxt,spacetxt(1:3)},...
                 'UserData',{4,3},...%for popupmenu cell array used to set initial values
                 'DataObject',[],...
                 'SelectedVar',{},...
                 'ActionButtons',{'Select','Close'},...
                 'Position',[]};        
    selection = inputUI.getUI(varargin{:});
    if isempty(selection), selection = {1,1}; end %use Matlab defaults

    hf = figure('Tag','PlotFig');
    t = tiledlayout(hf,mrows,ncols,'TileSpacing',spacetxt{selection{1}},...
                                   'Padding',spacetxt{selection{1}});
