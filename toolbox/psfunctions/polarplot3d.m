...
% 5.0  IHT added 'Axes' to control which axes is used. Changed all holds to
%      be axes specific, e.g. hold(p.axes,'on')
% 
...
% Draw an example in a new figure and display help text
    ...
    p.axes              = gca;        %default axes if not supplied--iht addition
    ...
    if isempty(azmgrid) || (isscalar(azmgrid) && azmgrid == 0)
       azmgrid = 1;
    end
    if isscalar(azmgrid)
    ...
    if isempty(radgrid) || (isscalar(radgrid) && radgrid==0)
       radgrid = 1;
    end
    if isscalar(radgrid)
    ...
    %check if axes included in call--iht addition--------------------------
...
function P = struct2pv(S)
    ...
    if ~isstruct(S), P = {}; return; end  % Check input argument
