...
%   alternative to get_selection which allows variables or dimensions to be 
%   selected using muiCatalogue.getProperty
% SEE ALSO
...
% 
    ...
    [sel.cobj,sel.classrec] = getCase(mobj.Cases,UIsel(1).caserec); %selected class instance
    dsnames = fieldnames(sel.cobj.Data);
    props.dset = dsnames{UIsel(1).dataset};
    dst = sel.cobj.Data.(props.dset); %selected dstable
    ...
    if UIsel(1).scale>1
        ...
        props.desc = sprintf('%s-%s',sel.scale,props.desc); 
    end
end
%%
function [data,attrib,dvals] = subselectData(dst,UIsel,mobj)
    ...
    [idx,dvals] = getSelectedIndices(mobj.Cases,UIsel,dst,attrib.names);  %extracts array for selected variable
