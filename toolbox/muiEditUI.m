...
%%
        function saveData(obj,mobj,newtable,idrows)
            ...
            %of the data selected (indices are row ids in full table)
            muicat = mobj.Cases;
            UIsel = obj.UIselection; 
...
%--------------------------------------------------------------------------
    methods (Access=private)
        function editDimension(obj,mobj,props)
            %edit row or dimension vector
            muicat = mobj.Cases;     %handle to model catalogue
            UIsel = obj.UIselection; %user selections

            subtable = props.data; %data defined by selection - can be a subset
            iddims = props.idx;    %can be row or dimension
            [subtable,type,format] = var2str(subtable);

            %make dimension a table 
            varname = props.attribs.names{UIsel.property};
            if strcmp(varname,'RowNames'), varname = 'Rows'; end  %avoid reserved name conflict            
            subtable = table(subtable,'VariableNames',{varname});
            ...
            %convert output back to input data type
            subtable = str2var(subtable{:,:},type,format);

            %selected case object to be editied
            UIsel = obj.UIselection; 
            [cobj,~,~] = getCase(muicat,UIsel.caserec);  
            ds = fieldnames(cobj.Data);
            dst = cobj.Data.(ds{UIsel.dataset});  %selected dataset

            if UIsel.property==2  %rows being edited
                dst.RowNames(iddims) = subtable;
                %update row range
            else   %dimension being edited, range updated on assignment
                dst.Dimensions.(varname)(iddims) = subtable;
            end
